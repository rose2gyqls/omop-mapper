"""
2026 Condition 엑셀 데이터 매핑 테스트 스크립트

데이터: data/2026_condition.xlsx의 "원내코드명" 열
도메인: Condition
...
class ConditionMappingTester:
    """2026 Condition 데이터 매핑 테스트 클래스"""
    
    def __init__(self, log_dir: str = "test_logs_2026_condition"):
        """테스터 초기화"""
    ...
    def setup_logging(self):
        ...
        log_file = self.log_dir / f"condition_mapping_test_{timestamp}.log"
        
        self.logger = logging.getLogger('condition_mapping_test')
    ...
    def load_excel_data(self, excel_path: str) -> pd.DataFrame:
        ...
        # 원내코드명 열 확인
        if '원내코드명' not in df.columns:
            raise ValueError("'원내코드명' 컬럼이 존재하지 않습니다.")
        
        # 중복 제거 및 빈 값 제외
        df_clean = df[df['원내코드명'].notna()].copy()
        df_clean = df_clean.drop_duplicates(subset=['원내코드명'])
    ...
    def create_entity_input(self, entity_name: str) -> EntityInput:
        """엔티티명으로부터 EntityInput 생성 (도메인: Condition)"""
        return EntityInput(
    ...
    def run_mapping_test(self, excel_path: str) -> dict:
        """매핑 테스트 실행"""
        self.logger.info("\n" + "=" * 100)
        self.logger.info("🧪 2026 Condition 매핑 테스트 시작")
        self.logger.info("=" * 100)
        self.logger.info("테스트 조건:")
        self.logger.info("  - Domain: Condition")
        ...
        for idx, row in tqdm(test_data.iterrows(), total=len(test_data), desc="매핑 테스트"):
            try:
                entity_name = str(row['원내코드명']).strip()
                kcd_code = str(row.get('KCD(ICD)', '')).strip() if 'KCD(ICD)' in row else None
                
                entity_input = self.create_entity_input(entity_name)
                
                result = self.test_single_entity(api, entity_input, kcd_code)
        ...
        summary = {
            'test_name': '2026_condition_mapping',
    ...
    def save_results(self, summary: dict):
        ...
        summary_csv = self.log_dir / f"condition_mapping_summary_{timestamp}.csv"
        summary_df.to_csv(summary_csv, index=False, encoding='utf-8')
        self.logger.info(f"📄 요약 CSV 저장: {summary_csv}")
        
        # 2. 상세 결과 XLSX 저장
        xlsx_file = self.log_dir / f"condition_mapping_detailed_{timestamp}.xlsx"
        ...
        # 3. JSON으로 전체 결과 저장
        json_file = self.log_dir / f"condition_mapping_results_{timestamp}.json"
        json_data = {
            'timestamp': timestamp,
    ...
    def _create_detail_sheet(self, ws, summary):
        """상세 결과 시트 생성"""
        headers = [
            "Entity Name", "KCD Code", "Domain", "Success",
        ...
        for row, result in enumerate(summary['results'], 2):
            ...
            ws.cell(row=row, column=2, value=result.get('kcd_code', 'N/A'))
            ws.cell(row=row, column=3, value=result.get('input_domain', 'N/A'))
            
            # 성공 여부 컬러링
            success_cell = ws.cell(row=row, column=4, value="성공" if result.get('success') else "실패")
            ...
            ws.cell(row=row, column=5, value=result.get('best_concept_id', 'N/A'))
            ws.cell(row=row, column=6, value=result.get('best_concept_name', 'N/A'))
            ws.cell(row=row, column=7, value=result.get('vocabulary_id', 'N/A'))
            ws.cell(row=row, column=8, value=result.get('best_score', 0.0))
            ws.cell(row=row, column=9, value=result.get('best_confidence', 'N/A'))
            ws.cell(row=row, column=10, value=result.get('mapping_method', 'N/A'))
            
            # Validation changed 여부 컬러링
            validation_changed = result.get('validation_changed', False)
            val_cell = ws.cell(row=row, column=11, value="변경됨" if validation_changed else "유지")
            ...
            if llm_top:
                ...
                llm_top_text = "N/A"
            ws.cell(row=row, column=12, value=llm_top_text)
            ws.cell(row=row, column=12).alignment = Alignment(wrap_text=True, vertical='top')
            
            # Error 정보
            error_text = result.get('error', '')
            ws.cell(row=row, column=13, value=error_text if error_text else "")
        ...
        # 열 너비 설정
        column_widths = [45, 12, 12, 10, 18, 50, 15, 10, 12, 20, 12, 40, 30, 70, 70, 85]
...
def main():
    ...
    # 설정
    # ============================================================
    EXCEL_PATH = "/home/work/skku/hyo/omop-mapper/data/2026_condition.xlsx"
    
    # ============================================================
    # 테스트 실행
    # ============================================================
    tester = ConditionMappingTester()
    
    results = tester.run_mapping_test(
        excel_path=EXCEL_PATH
    )
    
    print(f"\n✅ 2026 Condition 매핑 테스트 완료!")
